...
# --- 1. 基本設定與資料讀取 ---
st.set_page_config(page_title="智慧財務顧問 v14", layout="centered")
EXPENSE_FILE = 'expenses_v2.csv'
CARD_FILE = 'cards_v2.csv'

# 修正：確保讀取時強制轉換日期格式，避免 .dt 報錯
...
this_month_personal_spent = st.session_state.expenses[this_month_mask & (st.session_state.expenses['公司費用'] == False)]['金額'].sum()
...
current_liquid = user_salary - user_fixed - user_saving - this_month_personal_spent
daily_budget = current_liquid / days_left if days_left > 0 else 0

# --- 4. 儀表板看板 ---
st.title("💰 智慧財務顧問 v14")
c1, c2, c3 = st.columns(3)
c1.metric("本月可用預預算", f"${current_liquid:,.0f}")
...
if not st.session_state.expenses.empty:
    if col_btn.button("🧹 一鍵結算", use_container_width=True):
